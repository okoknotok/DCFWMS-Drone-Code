...
function handleBuilderClick(gx, gz) {
  ...
  } else if (builderTool === 'vase' || builderTool === 'plant') {
...
function placeObstacle(gx, gz, type) {
  ...
  builderObstacles.push({ gx, gz, type });
  rebuildBuilderObstacles();
  toast(`${type === 'vase' ? '🏺' : '🌿'} 已放置`, 'success');
}

function placeCheckpoint(gx, gz) {
  ...
  const label = String.fromCharCode(64 + builderCheckpointIdx); // A, B, C...
...
function rebuildBuilderObstacles() {
  // Clear old
  builderMeshes.filter(m => m._builderType === 'obstacle').forEach(m => m.dispose());
  builderMeshes = builderMeshes.filter(m => m._builderType !== 'obstacle');
  
  // Create new
  builderObstacles.forEach((obs, idx) => {
    let mesh;
    if (obs.type === 'vase') {
      mesh = createVase(obs.gx, obs.gz, idx);
    } else {
      mesh = createPlant(obs.gx, obs.gz, idx);
    }
...
function rebuildBuilderCheckpoints() {
  builderMeshes.filter(m => m._builderType === 'checkpoint').forEach(m => m.dispose());
  builderMeshes = builderMeshes.filter(m => m._builderType !== 'checkpoint');
  
  builderCheckpoints.forEach((cp, idx) => {
    const mesh = createCheckpoint(cp, idx);
...
function rebuildBuilderTreasures() {
  builderMeshes.filter(m => m._builderType === 'treasure').forEach(m => m.dispose());
  builderMeshes = builderMeshes.filter(m => m._builderType !== 'treasure');
  
  builderTreasures.forEach((t, idx) => {
    const mesh = createTreasure(t, idx);
...
function builderClearMeshes() {
  builderMeshes.forEach(m => m.dispose());
  builderMeshes = [];
}
...
function importShareCode() {
  ...
  builderCheckpointIdx = builderCheckpoints.length;
